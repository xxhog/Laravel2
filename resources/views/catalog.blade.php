...
<div class="row gy-4">
    @forelse($products as $product)
        <div class="col-6 col-md-4 col-lg-3">
            <div class="card h-100 border-0 shadow-sm product-card">
                ...
                {{-- ССЫЛКА НА КАРТОЧКУ (Обертка фото) --}}
                <a href="{{ route('landing.product', $product->id) }}" class="position-relative p-3 bg-light d-block text-center" style="border-radius: 20px 20px 0 0;">
                    @php
                        // Чистим путь от "public/", как и на главной
    ...
    @empty
        {{-- Блок "Товары не найдены" остается без изменений --}}
    @endforelse
</div>
</div>

<style>
        /* Эффект увеличения картинки внутри карточки */
    .transition-zoom {
        transition: transform 0.5s ease;
    }

    .product-card:hover .transition-zoom {
        transform: scale(1.1); /* Картинка чуть-чуть увеличивается при наведении на карточку */
    }

    /* Фикс для одинаковой высоты фото */
    .position-relative.p-3 {
        display: flex;
        align-items: center;
        justify-content: center;
    }
    /* Тот самый эффект как в контактах */
    .product-card {
    ...
    .product-card:hover {
        ...
        shadow: 0 1rem 3rem rgba(0,0,0,.175)!important; /* Усиление тени */
        box-shadow: 0 10px 25px rgba(13, 110, 253, 0.1); /* Легкий синий оттенок тени */
    }

    /* Анимация кнопки при наведении на карточку */
    ...
    /* Ограничение текста названия, чтобы карточки были одной высоты */
    .card-title {
        line-height: 1.2;
    }

    /* Для мобильных устройств: 2 карточки в ряд */
